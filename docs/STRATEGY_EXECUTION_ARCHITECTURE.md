...
Configuration handling lives in `Strategies/Grid/Configuration`: `GridConfigurationMapper` maps strategy and candidate inputs; `GridConfigurationService` validates saved settings and prepares preview/startup configuration-plan pairs; `GridConfigurationCodec` converts saved and frozen JSON without validation, metadata refresh, or database writes. Request validation stays before account/entity checks; saved-plan validation stays after them. Save/update uses selected instrument rules without persisting metadata, previews incorporate metadata and fees, and only `CURRENT_MID` startup refreshes them from the preflight quote. Manual startup retains its confirmed preview. Runtime consumers read only the cycle's frozen settings; `GridInstrumentRules` reconstructs instrument limits with the historical fallbacks. Editing a strategy affects future cycles, including automatic restarts, while each running cycle retains its frozen plan, settings, execution binding, and movement offset.
