...
import type { Alert, Cycle } from '../types'
...
export function AlertsPage({ notify, reportError }: { notify: (message: string) => void; reportError: (message: string) => void }) {
  ...
  async function load() {
    ...
    try {
      const [rows, history] = await Promise.all([api.alerts(), api.cycles()])
      if (current !== sequence.current) return
      setAlerts(rows); setCycles(history)
    } catch (e) { if (current === sequence.current) reportError(e instanceof Error ? e.message : '告警加载失败') }
  }
  useEffect(() => { void load(); const timer = setInterval(() => void load(), 5000); return () => { sequence.current++; clearInterval(timer) } }, [])
  ...
  return <div className="page padded alerts-page"><div className="page-title"><div><h1>风险与告警控制中心</h1><p>实时监控系统状态、持仓敞口及策略异常告警。</p></div><div><button className="secondary"><Icon name="download" size={16} /> 导出日志</button></div></div>
    ...
    <section className="panel alert-log"><header><h2>系统告警日志 <small>ALERT LOG</small></h2><div>{['全部', 'CRITICAL', 'WARNING', 'INFO'].map(x => <button key={x} className={filter === x ? 'active' : ''} onClick={() => setFilter(x)}>{x}</button>)}</div></header>
      <div className="table-wrap"><table><thead><tr><th>级别</th><th>时间</th><th>账户 / 环境</th><th>错误码</th><th>原因</th><th>系统行为</th><th>状态 / 操作</th></tr></thead><tbody>{rows.map(x => <tr key={x.id}><td><span className={`severity ${x.severity.toLowerCase()}`}><i />{x.severity}</span></td>
        <td className="mono">{new Date(x.createdAt).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}</td><td><AccountLabel accountId={bindings.get(x.cycleId ?? '')?.executionAccountId} environmentId={bindings.get(x.cycleId ?? '')?.executionEnvironmentId} /></td><td className="mono dim">{x.code}</td><td><b>{x.message}</b></td><td>{action(x.code)}</td>
