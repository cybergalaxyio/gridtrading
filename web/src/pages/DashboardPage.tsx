...
export function DashboardPage({ strategies, reload, notify, reportError }: {
  ...
  async function refresh() {
    try {
      ...
      if (strategy && executionEnvironmentId(strategy) === 'hyperliquid-testnet') { const book = await api.testnetBook(strategy.symbol); setTestnetMid(book.mid) } else setTestnetMid(null)
    } catch (e) { reportError(e instanceof Error ? e.message : '控制台数据加载失败') }
  }
  useEffect(() => { void refresh(); const timer = setInterval(() => void refresh(), 5000); return () => clearInterval(timer) }, [cycle?.cycleId, strategy?.strategyId, strategy?.defaultExecutionAccountId, strategy?.symbol])
  ...
  async function startCycle() {
    ...
    setBusy(true)
    try {
      const isTestnet = executionEnvironmentId(strategy) === 'hyperliquid-testnet'
      ...
      const preview = await api.preview(strategy.strategyId, strategy.version, quote.mid,
        executionEnvironmentId(strategy), strategy.defaultExecutionAccountId)
...
function executionEnvironmentId(strategy: Strategy) { return strategy.activeCycle?.executionEnvironmentId ?? strategy.defaultExecutionEnvironmentId }
