...
export function SettingsPage() {
  ...
  useEffect(() => {
    void Promise.all([api.hyperliquidAccounts(), api.hyperliquidAccounts('hyperliquid-mainnet')]).then(rows => rows.flat()).then(async rows => {
      ...
      if (checks.some(result => result.status === 'rejected')) setError('Some account checks failed. Verify credentials and network connectivity.')
    }).catch(e => setError(e instanceof Error ? e.message : 'Hyperliquid 状态读取失败'))
  }, [])
  ...
  return <div className="settings-page"><aside className="settings-nav"><h2>SETTINGS</h2>{['系统状态', '交易所账户', '通知设置', '界面设置', '审计与监控'].map((x, i) => <button className={i === 1 ? 'active' : ''} key={x}><Icon name={i === 0 ? 'dashboard' : i === 1 ? 'strategy' : i === 4 ? 'alert' : 'settings'} size={18} />{x}</button>)}</aside>
    <div className="settings-content"><div className="page-title"><div><h1>交易所账户</h1><p>API Wallet 按账户网络签名；查询始终使用主账户公开地址。</p></div></div>
