...
export function GridPreview({ levels, center, tickSize, quantityStep, symbol, indicative = false }: {
  levels: GridPreviewLevel[]; center: string; tickSize?: string | null; quantityStep?: string | null; symbol: string; indicative?: boolean
}) {
  if (!levels.length || !tickSize || !quantityStep) return <div className="grid-level-preview empty-grid"><span>{indicative ? '生成预览后显示参考 Grid；启动时使用最新 Bid / Ask' : '固定中心和交易规则就绪后显示 Grid Preview'}</span></div>
  const sells = levels.filter(level => level.side === 'SELL').sort((a, b) => b.price - a.price)
  const buys = levels.filter(level => level.side === 'BUY').sort((a, b) => b.price - a.price)
  ...
  return <div className="grid-level-preview"><header><span>LEVEL</span><b>PRICE</b><em>LOT SIZE</em></header><div className="grid-level-scroll">
    {sells.map(row)}<div className="grid-center-row"><span>C</span><b>{center}</b><em>{indicative ? 'PREVIEW MID' : 'FIXED CENTER'}</em></div>{buys.map(row)}
  </div><footer>{levels.length} 层 · Lot Size 已按 {quantityStep} 规范化</footer></div>
}

// Snap only floating-point noise at an integer tick/quantity boundary. A fixed
// EPSILON added to the price is too small at normal market-price magnitudes.
function stepUnits(value: number, step: number) {
  const units = value / step
  const nearest = Math.round(units)
  const tolerance = Number.EPSILON * Math.max(1, Math.abs(units)) * 4
  return Math.abs(units - nearest) <= tolerance ? nearest : units
}
function roundDown(value: number, step: number) { return Math.floor(stepUnits(value, step)) * step }
function roundUp(value: number, step: number) { return Math.ceil(stepUnits(value, step)) * step }
